      ...
      d3.json('nc_counties.json', function (error, data) {
        ...
        function jack(d) {
          ...
          d3.csv('income.csv', function (d) {
            // returns two columns for all coounties
            return {
              csvID: d['GEO.id2'],
              number: d.HC01_EST_VC15,
            }
          }, function (error, rows) {
            ...
            rows.forEach(function (element, item) {
              if (z == element.csvID) {
                number = rows[item].number;
              }
            })
            console.log(number);
            d3.select("#place")
              .text("Mean Household Income:  " + number)
              .attr('dx', '250')
              .attr('dy', '40')
              // .attr('id', 'text2')
              .attr('font-size', '20px')
          ...
          // d3.csv('ethnicity.csv', function (d) {
          //   return {
          //     csvID: d['GEO.id2'],
          //     white: d.HD01_VD02,
          //     black: d.HD01_VD03
